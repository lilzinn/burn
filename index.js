...
/*👆API KEY EX TEAM👆 adquira a sua key com*/o = 'wa.me/+557388843631'

//_INFORMAÇÕES DO BOT(settings)
const infos = JSON.parse(fs.readFileSync('./data/settings.json'))
const { prefix, ownerName, ownerNumber, cdd, ammOff, cr, crfig, crlv, crtt, crt, crh } = infos
const blocked = []
...
async function starts() {
	const burn = new WAConnection()
	burn.logger.level = 'warn'
	console.log(banner.string)
	burn.on('qr', () => {
		console.log(color('👆'), color(' Escanear o código acima para iniciar o burn-BOT!'))
	})

	fs.existsSync('./burn.json') && burn.loadAuthInfo('./burn.json')
	burn.on('connecting', () => {
		start('2', 'Conectando o burn-BOT...')
	})
	burn.on('open', () => {
		success('2', 'burn-BOT conectado!!!')
	})
	await burn.connect({timeoutMs: 30*1000})
        fs.writeFileSync('./burn.json', JSON.stringify(burn.base64EncodedAuthInfo(), null, '\t'))

//_FUCTION DE BOAS-VINDAS
burn.on('group-participants-update', async (anu) => {
		if (!welkom.includes(anu.jid)) return
		try {
			const mdata = await burn.groupMetadata(anu.jid)
			console.log(anu)
			if (anu.action == 'add') {
				...
				burn.sendMessage(mdata.id, welt, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			} else if (anu.action == 'remove') {
				num = anu.participants[0]
				try {
					ppimg = await burn.getProfilePicture(`${num.split('@')[0]}@c.us`)
				} catch {
					ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
				}
				teks = `O integrante @${num.split('@')[0]} saiu do grupo... bye bye👋`
				let buff = await getBuffer(ppimg)
				burn.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
			console.log('Error : %s', color(e, 'red'))
		}
	})
    burn.on('CB:Blocklist', json => {
      ...
      burn.on('chat-update', async (mek) => {
		try {
            ...
            const testat = budy
            const totalchat = await burn.chats.all()
			const botNumber = burn.user.jid
			const ownerNumber = [`557499510904@s.whatsapp.net`]
			const ownerNumberB = []
			const isGroup = from.endsWith('@g.us')
			const sender = isGroup ? mek.participant : mek.key.remoteJid
			const groupMetadata = isGroup ? await burn.groupMetadata(from) : ''
			...
			let pushname = burn.contacts[sender] != undefined ? burn.contacts[sender].vname || burn.contacts[sender].notify: undefined
    
			const reply = (tej) => {
				burn.sendMessage(from, tej, text, {quoted:mek})
			}
			const sendMess = (teks) => {
				burn.sendMessage(from, teks, text)
			}
            const sendImage = (tis) => {
		        burn.sendMessage(from, tis, image, {quoted:mek})
            }
			const mentions = (ops, memberr, sender, id) => {
				(id == null || id == undefined || id == false) ? burn.sendMessage(from, ops.trim(), extendedText, {contextInfo: {"mentionedJid": memberr}}) : burn.sendMessage(from, ops.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": memberr}})
			}
			const costum = (pesan, tipe, target, target2) => {
                 burn.sendMessage(from, pesan, tipe, {quoted: {key: {fromMe: false, participant: `${target}`, ...(from ? {remoteJid: from}: {})}, message: {conversation: `${target2}` }}})
            }
             const sendPtt = (teks) => {
                 burn.sendMessage(from, audio, mp3, {quoted: mek })
            }
...
burn.sendMessage(from, lvup, text, {quoted: mek})
//burn.sendMessage(from, lvup, text, {quoted: mek, contextInfo: {mentionedJid: [sender]}}
}
} catch (err) {
console.error(err)
}
...
burn.updatePresence(from, Presence.composing)
var kic = `${sender.split("@")[0]}@s.whatsapp.net`
reply(`Link de grupos não são permitidos aqui${sender.split("@")[0]}`)
setTimeout( () => {
burn.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
}, 1000)
setTimeout( () => {
burn.updatePresence(from, Presence.composing)
reply("adios")
...
burn.sendMessage(tttset.local,`❌ O TEMPO DE JOGO EXPIROU ❌\n\n➣  PUNIÇÃO: ${randomEndTTTXP} XP 🔮`, text, {quoted: tttset.mentionPlayer})
} else {
burn.sendMessage(tttset.local,`❌ O TEMPO DE JOGO EXPIROU ❌`, text, {quoted: tttset.mentionPlayer})
}
...
burn.sendMessage(from, buf, audio, {mimetype: 'audio/mp4', quoted: mek, ptt: true})
}
...
burn.sendMessage(from, cleitu, sticker, {quoted: mek})
}
...
burn.sendMessage(from, result, sticker, {quoted: mek })
} else if (budy.match('burn')) {
result = fs.readFileSync(`./src/mask.webp`)
burn.sendMessage(from, result, sticker, {quoted: mek })
} else if (budy.match('burn')) {
result = fs.readFileSync(`./src/mask.webp`)
...
const foto = fs.readFileSync('src/menu1.png')
...
burn.sendMessage(from, mz(prefix), text, tescuk, cr)
break

case 'gc':
if (body.endsWith('Tutup')){
  burn.groupSettingChange (from, GroupSettingChange.messageSend, true)
}
break
					...
					case 'lb':
						...
						try {
					     for (let i = 0; i < 3; i++) {
							...
							console.log(err)
							burn.sendMessage(from, `É necessário que no mínimo 3 pessoas tenham level...`, text, {quoted: mek})
						}
					break

case 'ts':
burn.sendMessage(from, bahasa(prefix, sender), text, {
quoted: mek
})
break
...
const bug = body.slice(10)
 if (args.length > 300) return burn.sendMessage(from, 'Máximo 300 caracteres', msgType.text, {quoted: mek})
...
burn.sendMessage('557499510904@s.whatsapp.net', options, text, {quoted: mek})
...
burn.sendMessage(from, `${anu.success} 🐤`, text, {quoted: mek})
} catch {
reply(ptbr.erro)
}
break
...
burn.sendMessage(from, anu, image, {quoted: mek})
break
...
burn.sendMessage(from, `https://github.com/italuH/burn-BOT` , text, {quoted: mek})
break

case 'macumba':
pedido = body.slice(8)
buf = fs.readFileSync(`./src/macu.mp4`)
burn.sendMessage(from, buf, video, {quoted: mek})
break
...
burn.sendMessage(_.jid, aMimirMp3, MessageType.audio, {mimetype: 'audio/mp4', ptt:true})
burn.sendMessage(_.jid, aMimirSt, sticker)
}
ammOff = "on"
...
burn.sendMessage(from, daftarfile, document, {mimetype: 'json', filename: 'daftar.json'})
break
...
burn.sendMessage(nomor+'@s.whatsapp.net', pesan, text)
break
...
burn.sendMessage(from, attp2, sticker, {quoted: mek})
break
...
╰╼╾╼╾╼╾╼╾╼╾╼╾╼
  ...
  ════❖ LEVEL ❖════`     
burn.sendMessage(from, levela, text, {quoted: mek, contextInfo: {mentionedJid: [sender]}})
break
...
case 'italu':
case 'italo':
await burn.sendMessage(from, {displayname: "Italu🧙‍♂️", vcard: vcard}, MessageType.contact)
break
...
burn.sendMessage(from, options, text, { quoted: mek } )
break
...
case 'testime':
setTimeout( () => {
burn.sendMessage(from, 'O tempo acabou', text) 
}, 10000)
setTimeout( () => {
burn.sendMessage(from, '5 segundos restantes', text)
}, 5000)
setTimeout( () => {
burn.sendMessage(from, '10 segundos restantes', text)
}, 0)
break
...
burn.sendMessage(from, buf, image, {quoted: mek,caption: "🐱"})
})})
} catch (e) {
console.log(`Error :`, color(e,'red'))
reply('❌ocorreu um erro❌\n\nTente novamente. ')
...
burn.sendMessage(from, buf, MessageType.image, {quoted: mek})
})
})
} catch (e) {
...
reply('A mimir...')
exec(pm2, (err, stdout) => {
if(err) return burn.sendMessage(from, "Erro", text, {quoted: mek})
if (stdout) {
burn.sendMessage(from, stdout, text, {quoted: mek})
} 
})
break
...
exec(cmd, (err, stdout) => {
if(err) return burn.sendMessage(from, "Comando inexistente", text, {quoted: mek})
if (stdout) {
burn.sendMessage(from, stdout, text, {quoted: mek})
}
})
...
burn.sendMessage(from, teks, text, {quoted: mek})
...
burn.sendMessage(from, buffer, image, {quoted: mek})
teks = '𝗣𝗹𝗮𝘆 𝘀𝘁𝗼𝗿𝗲\n'
...
dung = (anu.sticker)
burn.sendMessage(from, buffer, image, {quoted: mek, caption: teks})
...
burn.sendMessage(from, buffer, image, {quoted: mek, caption: teks.trim()})
break
...
const mediaaa = await burn.downloadAndSaveMediaMessage(encmediaaa)
reply(ptbr.wait())
a = await webp2gifFile(mediaaa)
mp4 = await getBuffer(a.result)
burn.sendMessage(from, mp4, MessageType.video, {mimetype: 'video/gif', filename: `stick.gif`, quoted: mek, caption: '✅'})
...
burn.sendMessage(from, buff, image, {quoted: mek, caption: '✏📕'})
break
...
try {
ppimg = await burn.getProfilePicture(`${sender.split('@')[0]}@c.us`)
} catch {
ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
}
const pf = 
...
burn.sendMessage(from, its, image, {quoted: mek, caption: pf, contextInfo: {mentionedJid: [sender]}})
...
media = await burn.downloadAndSaveMediaMessage(encmedia)
ran = getRandom('.mp4')
exec(`ffmpeg -i ${media} -vf reverse -af areverse ${ran}`, (err) => {
fs.unlinkSync(media)
if (err) return reply(`Err: ${err}`)
buffer453 = fs.readFileSync(ran)
burn.sendMessage(from, buffer453, video, { mimetype: 'video/mp4', quoted: mek })
fs.unlinkSync(ran)
})
break
...
burn.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt:true, quoted: mek})
fs.unlinkSync(ran)
})
break   
...
slo = await burn.downloadAndSaveMediaMessage(low)
ran = getRandom('.mp3')
...
tup = await burn.downloadAndSaveMediaMessage(pai)
ran = getRandom('.mp3')
...
gem = await burn.downloadAndSaveMediaMessage(muk)
ran = getRandom('.mp3')
...
bas = await burn.downloadAndSaveMediaMessage(ass)
ran = getRandom('.mp3')
...
inf =
 `‣ Nome do bot: ${me.name}
  ...
  ‣ Criador: https://youtube.com/channel/UC1lsMlGC6OI_llwN0xeEXZA
  ‣ Prefix: ${prefix}
  ‣ Base: Kratosツ
...
burn.sendMessage(from, buffer, image, {caption: inf, contextInfo: {mentionedJid: [me]}})
break
...
try {
ppUrl = await burn.getProfilePicture(from)
} catch {
...
` 
  ...
  ‣Descrição:
  ${desc ? desc : ''}`
await burn.sendMessage(from, buffer, image, {quoted: mek, caption: infogp, contextInfo: {mentionedJid: [owner.split]}})
break
...
burn.sendMessage(from, pok, image, {quoted: mek, caption: `Achei isso sobre: ${args}`})
} catch {
...
let online = [...Object.keys(burn.chats.get(ido).presences), burn.user.jid]
burn.sendMessage(from, 'Lista de usuários online:\n' + online.map(v => '- @' + v.replace(/@.+/, '')).join`\n`, text, { quoted: mek,
...
nimek = n[Math.floor(Math.random() * n.length)];
pok = await getBuffer(nimek)
burn.sendMessage(from, pok, image, {
...
const medipp = await burn.downloadAndSaveMediaMessage(ftgp)
await burn.updateProfilePicture (from, medipp)
...
reply(ptbr.waitimg())
owgi = await burn.downloadAndSaveMediaMessage(ger)
...
burn.sendMessage(from, nobg, sticker, {quoted: mek})
fs.unlinkSync(rano)
})                               
} else {
...
burn.sendMessage(from, hehe, image, {quoted:mek})
} else {
...
reply(ptbr.waitimg())
owgi = await burn.downloadAndSaveMediaMessage(ted)
tels = body.slice(7)
anu = await imgbb("0c419be2e8bfc27eff00147b0c763418", owgi)
...
burn.sendMessage(from, heh, image, {quoted:mek})
break
///_photoOXY
...
` INFORMAÇÕES DO CEP
  ...
  ‣ Estado: ${hehe.state}
  ‣ Cidade: ${hehe.city}`
burn.sendMessage(from, ccg, text, {quoted:mek})
break
...
burn.sendMessage(from, options, text, {
quoted: mek
})
break
...
burn.updatePresence(from, Presence.composing)
if (!isGroup) return reply(ptbr.group())
if(args.length < 1) return burn.reply('escreva algo como aviso')
aviso  = `Aviso de: @${sender.split("@")[0]}\n\nAviso: ${body.slice(7)}`
group = await burn.groupMetadata(from);
...
await burn.sendMessage(from, options, text)
break
...
burn.sendMessage(from, tagme, text)
break
...
//aanu = await fetchJson(`https://api-burn.italuh.repl.co/api/yta?url=${anu.result.source}`)
...
setTimeout( () => {
burn.sendMessage(from, buffer, image, {quoted: mek, caption: infomp3})
}, 1500)
reply('Baixando e enviando sua música...')
burn.sendMessage(from, lagu, audio, {mimetype: 'audio/mp4', quoted: mek})
break
...
burn.sendMessage(from, buffer, image, {
...
`    DADOS GERADOS
   ...
   ‣ Senha: ${anu.password}
   ‣ CEP: ${anu.zip_code}`
burn.sendMessage(from, dadosf, text, {quoted: mek})
break
...
try {
anu = await fetchJson(`https://api-burn.italuh.repl.co/api/yta?url=${play}`)
...
burn.sendMessage(from, lagu, audio, {
...
burn.sendMessage(from, lagu, video, {
...
const com = await burn.downloadAndSaveMediaMessage(ocrt)
reply(ptbr.wait())
...
const media = await burn.downloadAndSaveMediaMessage(encmedia)
ran = getRandom('.webp')
await ffmpeg(`./${media}`)
.input(media)
...
.on('end', function () {
console.log('Finish')
exec(`webpmux -set exif ${addMetadata('Burn - Bot OFC', 'Stickers')} ${ran} -o ${ran}`, async (error) => {
if (error) return reply(ptbr.stick())
burn.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: mek})
fs.unlinkSync(media)	
fs.unlinkSync(ran)	
 })
 })
...
.on('end', function () {
console.log('Finish')
exec(`webpmux -set exif ${addMetadata('burn.sendMessage(from,  foto, image,  {quoted: mek, quoted: { key: { fromMe:  false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg","caption": "🌧️Burn - Bot OFC🌙", 'jpegThumbnail': fs.readFileSync('kk/sticker/botlogo.webp')}}}, caption: shiz(prefix)})')} ${ran} -o ${ran}`, async (error) => {
if (error) return reply(ptbr.stick())
burn.sendMessage(from, fs.readFileSync(ran), sticker, crt, {quoted: mek})
fs.unlinkSync(media)
fs.unlinkSync(ran)
})
})
...
fs.unlinkSync(ranp)
if (err) return reply(ptbr.stick())
exec(`webpmux -set exif ${addMetadata('Burn - Bot OFC', 'Stickers')} ${ranw} -o ${ranw}`, async (error) => {
if (error) return reply(ptbr.stick())
burn.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: mek})
fs.unlinkSync(ranw)
})
...
.on('error', function (err) {
console.log(`Error : ${err}`)
exec(`webpmux -set exif ${addMetadata('Burn - Bot OFC', 'Stickers')} ${rano} -o ${rano}`, async (error) => {
fs.unlinkSync(media)
reply(ptbr.stick())
})
})
...
burn.sendMessage(from, buffer, sticker, {quoted: mek})
fs.unlinkSync(rano)
})
} else if ((isMedia && mek.message.videoMessage.seconds < 11 || isQuotedVideo && mek.message.extendedTextMessage.contextInfo.quotedMessage.videoMessage.seconds < 11) && args.length == 0) {
const encmedia = isQuotedVideo ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
...
//_VOZ DO GOOGLE
case 'tts':
if (args.length < 1) return burn.sendMessage(from, `Você deve usar o comando da forma correta:\n${prefix}tts (língua) (texto)\nExemplo: ${prefix}tts pt bom dia\n\nUse: ${prefix}ts para listar todas as línguas`, text, {quoted: mek})
const gtts = require('./lib/gtts')(args[0])
if (args.length < 2) return burn.sendMessage(from, 'Cadê o texto?', text, {quoted: mek})
dtt = body.slice(8)
ranm = getRandom('.mp3')
dtt.length > 800
? reply('Texto muito grande')
: gtts.save(ranm, dtt, function() {
burn.sendMessage(from, fs.readFileSync(ranm), audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
fs.unlinkSync(ranm)
})
break
...
if (!isGroup) return reply(ptbr.group())
if (!isOwner) return reply(ptbr.ownerB())
burn.blockUser (`${body.slice(8)}@c.us`, "add")
burn.sendMessage(from, `Memblokir nomor, Perintah Diterima`, text, {
quoted: mek
})
break
...
if (!isGroup) return reply(ptbr.group())
if (!isOwner) return reply(ptbr.ownerB())
burn.blockUser (`${body.slice(9)}@c.us`, "remove")
burn.sendMessage(from, `Membuka blokir, Perintah diterima`, text)
break
...
anu = await burn.chats.all()
burn.setMaxListeners(25)
for (let _ of anu) {
burn.deleteChat(_.jid)
}
...
burn.groupUpdateSubject(idgrup, `${body.slice(9)}`)
burn.sendMessage(from, 'nome do grupo alterado', text, {
quoted: mek
})
break
...
if (!isGroupAdmins) return reply(ptbr.admin())
if (!isBotGroupAdmins) return reply(ptbr.Badmin())
burn.groupUpdateDescription(from, `${body.slice(9)}`)
burn.sendMessage(from, 'descrição do grupo alterada', text, {
quoted: mek
})
break
...
burn.groupDemoteAdmin(from, mentioned)
}
break
...
const cuk = await burn.downloadAndSaveMediaMessage(botpp)
await burn.updateProfilePicture(botNumber, cuk)
...
if (!isGroupAdmins) return reply(ptbr.admin())
if (!isBotGroupAdmins) return reply(ptbr.Badmin())
linkgc = await burn.groupInviteCode(from)
...
medtmg = await burn.downloadAndSaveMediaMessage(tomg)
ran = getRandom('.png')
exec(`ffmpeg -i ${medtmg} ${ran}`, (err) => {
fs.unlinkSync(medtmg)
if (err) return reply('❌falha ao converter sticker em imagem❌')
buffer = fs.readFileSync(ran)
burn.sendMessage(from, buffer, image, {quoted: mek, caption: 'conversão sucedida'})
fs.unlinkSync(ran)
})
break
...
veri = sender
try {
ppimg = await burn.getProfilePicture(`${sender.split('@')[0]}@s.whatsapp.net`)
} catch {
...
burn.sendMessage(from, daftarimg, image, {quoted: mek, caption: captionnya, contextInfo: {mentionedJid: [sender]}})                    
break
...
cpf = `${cp1}.${cp2}.${cp3}-${cp4}`
burn.sendMessage(from, `CPF gerado: ${cpf}`, text, {quoted: mek})
break
...
burn.groupSettingChange (from, GroupSettingChange.messageSend, false)
burn.sendMessage(from, open, text, {
  quoted: mek
})
break
...
try {
pp = await burn.getProfilePicture(id)
buffer = await getBuffer(pp)
burn.updateProfilePicture(botNumber, buffer)
...
burn.sendMessage(from, dador, sticker, {quoted: mek})
break
...
burn.sendMessage(from, cararoa, sticker, {quoted: mek})
break
...
const sn = ['sim', 'não']
gosto = body.slice(3)
if (args.length < 1) return burn.sendMessage(from, `Você deve fazer uma pergunta...\nExemplo: ${prefix}sn O Italu é um baiano preguiçoso?`, text, {quoted: mek})
...
if (vit == "derrota" ) {
var tes = "A vitória é do Burn-BOT"
}
if (vit == "empate" ) {
var tes = "O jogo terminou em empate"
}
reply(`burn-BOT jogou: ${pptb}\nO jogador jogou: ${args}\n\n${tes}`)
...
case 'ship':
if (!isGroup) return reply(ptbr.group())
const ag = await burn.getgroupMetadata.participants(from)
...
burn.sendMessage(from, casal, text, {quoted: mek, contextInfo: {"mentionedJid": [ag]}})
break
...
burn.sendMessage(from, slott, text, {quoted: mek})
break

case 'chance':
burn.updatePresence(from, Presence.composing) 
var avb = body.slice(7)
if (args.length < 1) return burn.sendMessage(from, `Você precisa digitar da forma correta\nExemplo: ${prefix}chance do Italu ser um trouxa`, text, {quoted: mek})
random = `${Math.floor(Math.random() * 100)}`
hasil = `A chance ${body.slice(7)}\n\né de... ${random}%`
burn.sendMessage(from, hasil, text, {quoted: mek, contextInfo: {mentionedJid: [sender]}})
break
...
setTimeout( () => {
burn.sendMessage(from, figb, sticker, {quoted: mek})
}, 2100)
}
setTimeout( () => {
burn.sendMessage(from, morte, text, {quoted: mek})
}, 2300)
break
...
burn.sendMessage(from, `🌀1️⃣2️⃣3️⃣\n🅰️${esp.a1}${esp.a2}${esp.a3}\n🅱️${esp.b1}${esp.b2}${esp.b3}\n©️${esp.c1}${esp.c2}${esp.c3}`,text )
burn.sendMessage(from,`Caso não saiba como jogar digite: ${prefix}ttthelp`, text) 
setTimeout( () => {
...
case 'ttthelp':
burn.sendMessage(from, ttthelp(prefix), text)
break
...
burn.sendMessage(from, tttme(pushname, getTTTwins(sender), getTTTdefeats(sender), getTTTties(sender), getTTTpoints(sender)), text, {quoted:mek})
break
...
console.log(err)
await burn.sendMessage(from, `Humm, é necessário que no mínimo 3 pessoas tenham jogado...`, text, {quoted: mek})
}
break
...
burn.sendMessage(from, `🎉🎉 VITÓRIA DO JOGADOR 🎉🎉\n\n➣  RECOMPENSA: +${randomTTTXP} XP 🔮`, text)
} else {
burn.sendMessage(from, `🎉🎉 VITÓRIA DO JOGADOR 🎉🎉`, text)
}
...
burn.sendMessage(from, `🎉🎉 VITÓRIA DO BURN-BOT 🎉🎉\n\n➣  PUNIÇÃO: ${randomTTTXP} XP 🔮`, text)
} else {
burn.sendMessage(from, `🎉🎉 VITÓRIA DO BURN-BOT 🎉🎉`, text)
}
...
if (isLevelingOn) {
burn.sendMessage(from, `🎉🎉 EMPATE 🎉🎉\n\n➣  NÃO HÁ GANHOS NEM PERDAS`, text)
} else {
burn.sendMessage(from, `🎉🎉 EMPATE 🎉🎉`, text)
}
...
imageToBase64(res.data.url).then((response) => {var buf = Buffer.from(response, 'base64');
burn.sendMessage(from, buf, image, {quoted: mek,caption: "KAWAII!!"})
})})
} catch (e) {
console.log(`Error :`, color(e,'red'))
reply('Ocorreu um erro')
...
axios.get(`https://nekos.life/api/v2/img/kemonomimi`).then((res)=>{
imageToBase64(res.data.url).then((response) => {var buf = Buffer.from(response, 'base64');
burn.sendMessage(from, buf, image, {quoted: mek})
})})
} catch (e) {
console.log(`Error :`, color(e,'red'))
reply('Ocorreu um erro')
...
burn.sendMessage(from, buffer, image, {quoted: mek, caption: 'Seu lolicon safado😔'})
})
} catch (e) {
console.log(`Error :`, color(e,'red'))
reply('❌ocorreu um erro❌\n\nTente novamente. ')
...
(ress) => {
var buf = Buffer.from(ress, 'base64')
burn.sendMessage(from, buf, MessageType.video, {mimetype: 'video/gif', filename: `stick.gif`, quoted: mek, caption: '✅'})
})
})
} catch (e) {
console.log(`Error :`, color(e,'red'))
...
burn.sendMessage(from, hsl, text, {quoted: mek, contextInfo: {mentionedJid: [sender]}})
}
}
} catch (e) {
console.log('Error : %s', color(e, 'red'))
