...
//_TIPO DE USUÁRIO
if (groupAdmins) {prema = 'Ademir🤓'}if (isPremium) {prema = 'Premium😎'} if (isOwner) {prema = 'El oddy🕴🏽'} if (isUser) {prema = 'Registrado 🥳'} else {var prema = 'Membro comum🗿'}
...
case 'pão':
if (isOwner) return reply('Oi ${ownerName}')
else {
...
case 'oddy':
case 'dono':
await tiringa.sendMessage(from, {displayname: "oddy🧙‍♂️", vcard: vcard}, MessageType.contact)
break
...
case 'clearall':
if (!isOwner) return reply('Só o oddy pode fazer isso')
...
if (!q.includes('|')) return reply(`Digite da forma correta:\nComando: ${prefix}registrar nome|idade\nExemplo: ${prefix}registrar ${pushname}|18`)
...
tiringa.sendMessage(from, `🎉🎉 VITÓRIA DO BURN-BOT 🎉🎉\n\n➣  PUNIÇÃO: ${randomTTTXP} XP 🔮`, text)
} else {
tiringa.sendMessage(from, `🎉🎉 VITÓRIA DO BURN-BOT 🎉🎉`, text)
}
